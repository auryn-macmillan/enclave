...
pub use uniswap_cca_example::{
    accumulate_demand, build_params, build_price_ladder, decode_demand_slot, PRICE_LEVELS, SLOTS,
};
...
pub fn compute_net_flow(buyer_allocations: &[u64], seller_allocations: &[u64]) -> i64 {
    let total_buy = i64::try_from(buyer_allocations.iter().sum::<u64>())
        .expect("total buyer allocation exceeds i64 range");
    let total_sell = i64::try_from(seller_allocations.iter().sum::<u64>())
        .expect("total seller allocation exceeds i64 range");
