...
for pair in np2 np3; do
  path="$RT/$pair"
  start=$(date +%s)
  (cd "$path" && taskset -c 0-3 timeout 420 /usr/bin/time -v stdbuf -oL -eL nargo compile > "$RT/${pair}_run.out" 2>&1)
  rc=$?
  end=$(date +%s)
  echo "${pair} wall=$((end-start))s nargo_rc=$rc" >> "$RT/leg2_status.log"
  if [ $rc -eq 0 ]; then
    (cd "$path" && nargo gates -o "$RT/${pair}_gates.json" 2> "$RT/${pair}_gates.stderr")
