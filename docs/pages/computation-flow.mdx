...
3. **Publish Data**: CRISP publishes the encrypted bytes to Avail and waits for VectorX to anchor the
   Avail block on Ethereum.
4. **Submit Inputs**: The E3 Program receives the content hash, the ZK proof, and the VectorX receipt.
   It accepts the input only when the receipt proves availability of the exact bytes named by the
   hash and the proof binds the same hash to the ciphertext commitment.
