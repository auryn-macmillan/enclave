...
> **Testnet tokens:** on Sepolia you can self-serve test FOLD and ticket collateral from the faucet
> with `interfold faucet` (requires the `faucet` contract address in your config — see
> [Running a Node → Contract Addresses](/ciphernode-operators/running#contract-addresses)). Each
> call tops up FOLD and the ticket-collateral token independently, only when your balance for that
> token is below the faucet threshold. One call dispenses enough FOLD and ticket collateral to meet
> the default Sepolia ciphernode requirements. If the faucet runs dry, reach out in the
...
| Ticket purchase fails     | Insufficient ticket collateral or allowance    | Ensure balance and approve spending                     |
