...
> **Testnet tokens:** on Sepolia you can self-serve test FOLD and fee tokens from the faucet with
...
> call tops up FOLD and the fee token independently when your balance is below the faucet threshold.
> The faucet does not dispense ticket collateral. To register a Sepolia ciphernode, get the separate
> test ticket-collateral token from the Interfold team. If the faucet runs dry, reach out in the
