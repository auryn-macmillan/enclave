...
use e3_zk_helpers::circuits::threshold::eval_key_relin_round1_share::computation::Inputs;

impl Provable for EvalKeyRelinRound1ShareCircuit {
    type Params = BfvPreset;
    type Input = EvalKeyRelinRound1ShareCircuitData;
    type Inputs = Inputs;

    fn circuit(&self) -> CircuitName {
        CircuitName::EvalKeyRelinRound1Share
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_c9_prove_smoke() {
        let Some(bb) = discover_compatible_bb().await else {
            eprintln!(
                "skipping C9 smoke test: compatible bb binary not found (need {})",
                REQUIRED_BB_VERSION
            );
            return;
        };
        ...
        fs::create_dir_all(
            circuits_dir
                .join("recursive")
                .join("threshold")
                .join("eval_key_relin_round1_share"),
        )
        .await
        .unwrap();
        ...
        let json_src = if target_build.join("eval_key_relin_round1_share.json").exists() {
            target_build.join("eval_key_relin_round1_share.json")
        } else {
            dist.join("recursive")
                .join("threshold")
                .join("eval_key_relin_round1_share")
                .join("eval_key_relin_round1_share.json")
        };
        let vk_src = if target_build
            .join("eval_key_relin_round1_share.vk_noir")
            .exists()
        {
            target_build.join("eval_key_relin_round1_share.vk_noir")
        } else if target_build
            .join("eval_key_relin_round1_share.vk_recursive")
            .exists()
        {
            target_build.join("eval_key_relin_round1_share.vk_recursive")
        } else if target_build.join("eval_key_relin_round1_share.vk").exists() {
            target_build.join("eval_key_relin_round1_share.vk")
        } else {
            dist.join("recursive")
                .join("threshold")
                .join("eval_key_relin_round1_share")
                .join("eval_key_relin_round1_share.vk")
        };

        if json_src.exists() && vk_src.exists() {
            fs::copy(
                &json_src,
                target.join("eval_key_relin_round1_share.json"),
            )
            .await
            .unwrap();
            fs::copy(&vk_src, target.join("eval_key_relin_round1_share.vk"))
                .await
                .unwrap();
        }
        ...
        let circuit_dir = prover
            ...
            .join(CircuitName::EvalKeyRelinRound1Share.dir_path());
        if !circuit_dir.join("eval_key_relin_round1_share.json").exists()
            || !circuit_dir.join("eval_key_relin_round1_share.vk").exists()
        {
            panic!(
                "C9 circuit not found at {} — build circuits and ensure dist/circuits includes threshold/eval_key_relin_round1_share",
                circuit_dir.display()
            );
        }

        let preset = BfvPreset::InsecureThreshold512;
        let sample = EvalKeyRelinRound1ShareCircuitData::generate_sample(preset)
            .expect("sample generation");
        let proof = EvalKeyRelinRound1ShareCircuit
            .prove(&prover, &preset, &sample, "c9-smoke")
            .expect("C9 prove should succeed");

        assert!(!proof.data.is_empty());
        assert!(!proof.public_signals.is_empty());

        let verified = EvalKeyRelinRound1ShareCircuit
            .verify(&prover, &proof, "c9-smoke", 0)
            .expect("C9 verify should succeed");
        assert!(verified, "C9 proof verification must pass");
    }
}
